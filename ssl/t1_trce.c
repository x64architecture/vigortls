...
#define ssl_trace_list(bio, indent, msg, msglen, value, table) \
    do_ssl_trace_list(bio, indent, msg, msglen, value, table,  \
                      sizeof(table) / sizeof(ssl_trace_tbl))
...
static int do_ssl_trace_list(BIO *bio, int indent, const uint8_t *msg,
                             size_t msglen, size_t vlen, ssl_trace_tbl *tbl,
                             size_t ntbl)
{
    int val;
    if (msglen % vlen)
...
static int ssl_print_hexbuf(BIO *bio, int indent, const char *name, size_t nlen,
                            const uint8_t **pmsg, size_t *pmsglen)
{
    size_t blen;
...
static int ssl_print_random(BIO *bio, int indent, const uint8_t **pmsg,
                            size_t *pmsglen)
{
    unsigned int tm;
    const uint8_t *p = *pmsg;
    ...
    switch (extype) {
        ...
        case TLSEXT_TYPE_renegotiate:
            ...
            if (xlen) {
                ...
                if (server) {
                    ext += xlen;
                    ssl_print_hex(bio, indent + 4, "server_verify_data", ext,
                                  xlen);
                }
            }
            else {
...
static int dtls_print_hello_vfyrequest(BIO *bio, int indent, const uint8_t *msg,
                                       size_t msglen)
{
...
static int ssl_print_server_hello(BIO *bio, int indent, const uint8_t *msg,
                                  size_t msglen)
{
    unsigned int cs;
    if (!ssl_print_version(bio, indent, "server_version", &msg, &msglen))
                                  ...
                                  const uint8_t *msg, size_t msglen)
{
    ...
    switch (id) {
        ...
        case SSL_kRSA:
            if (!ssl_print_hexbuf(bio, indent + 2, "EncyptedPreMasterSecret", 2,
                                  &msg, &msglen))
                return 0;
            break;
                                  ...
                                  const uint8_t *msg, size_t msglen)
{
    ...
    switch (id) {
        case SSL_kRSA:
            ...
            if (!ssl_print_hexbuf(bio, indent + 2, "rsa_modulus", 2, &msg,
                                  &msglen))
                return 0;
        ...
        case SSL_kECDHE:
            ...
            if (msg[0] == EXPLICIT_PRIME_CURVE_TYPE)
                ...
                msg += 3;
                msglen -= 3;
                if (!ssl_print_hexbuf(bio, indent + 2, "point", 1, &msg,
                                      &msglen))
                    return 0;
            }
            break;
...
static int ssl_print_certificate(BIO *bio, int indent, const uint8_t **pmsg,
                                 size_t *pmsglen)
{
    size_t msglen = *pmsglen;
    size_t clen;
    X509 *x;
    const uint8_t *p = *pmsg, *q;
    if (msglen < 3)
        return 0;
...
static int ssl_print_certificates(BIO *bio, int indent, const uint8_t *msg,
                                  size_t msglen)
{
    size_t clen;
    if (msglen < 3)
...
static int ssl_print_ticket(BIO *bio, int indent, const uint8_t *msg,
                            size_t msglen)
{
...
static int ssl_print_handshake(BIO *bio, SSL *ssl, const uint8_t *msg,
                               size_t msglen, int indent)
{
    size_t hlen;
    uint8_t htype;
    if (msglen < 4)
               ...
               size_t msglen, SSL *ssl, void *arg)
{
    ...
    BIO_printf(bio, "  Content Type = %s (%d)\n",
               ssl_trace_str(content_type, ssl_content_tbl), content_type);
