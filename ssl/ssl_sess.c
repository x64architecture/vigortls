...
SSL_SESSION *ssl_session_dup(SSL_SESSION *src, int ticket)
{
    ...
    dest->lock = CRYPTO_thread_new();
    if (dest->lock == NULL)
        goto err;

    if (src->sess_cert != NULL)
        SSL_CERT_up_ref(src->sess_cert);
                         ...
                         const uint8_t *limit)
{
    ...
    if (ret->sid_ctx_length != s->sid_ctx_length || memcmp(ret->sid_ctx, s->sid_ctx, ret->sid_ctx_length) != 0) {
...
void SSL_CTX_flush_sessions(SSL_CTX *s, long t)
{
    ...
    lh_SSL_SESSION_doall_arg(tp.cache, LHASH_DOALL_ARG_FN(timeout), TIMEOUT_PARAM,
                             &tp);
