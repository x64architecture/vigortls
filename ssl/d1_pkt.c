...
static int dtls1_process_buffered_records(SSL *s)
{
    ...
    if (item) {
        ...
        if (rb->left > 0) {
             ...
             * be a record from the new epoch in it - so don't overwrite it with
             * the unprocessed records yet (we'll do it when we've finished
...
start:
    ...
    if (rr->type == SSL3_RT_CHANGE_CIPHER_SPEC) {
        ...
        if (s->msg_callback)
            s->msg_callback(0, s->version, SSL3_RT_CHANGE_CIPHER_SPEC, rr->data,
                            1, s, s->msg_callback_arg);
