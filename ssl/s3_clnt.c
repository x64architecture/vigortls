...
int ssl3_get_server_done(SSL *s)
{
    ...
    n = s->method->ssl_get_message(
        s, SSL3_ST_CR_SRVR_DONE_A, SSL3_ST_CR_SRVR_DONE_B, SSL3_MT_SERVER_DONE,
...
static int ssl3_send_client_kex_rsa(SSL *s, SESS_CERT *sess_cert, uint8_t *p,
                               int *outlen)
{
...
static int ssl3_send_client_kex_dhe(SSL *s, SESS_CERT *sess_cert, uint8_t *p,
                               int *outlen)
{
...
static int ssl3_send_client_kex_gost(SSL *s, SESS_CERT *sess_cert, uint8_t *p,
                                int *outlen)
{
    uint8_t premaster_secret[32], shared_ukm[32], tmp[256];
    ...
    if (RAND_bytes(premaster_secret, sizeof(premaster_secret)) <= 0)
        goto err;
    ...
    *(p++) = V_ASN1_SEQUENCE | V_ASN1_CONSTRUCTED;
    ...
    s->session->master_key_length = s->method->ssl3_enc->generate_master_secret(
        s, s->session->master_key, premaster_secret, sizeof(premaster_secret));

    *outlen = n;
    ret = 1;

err:
    vigortls_zeroize(premaster_secret, sizeof(premaster_secret));
