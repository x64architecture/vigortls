                       ...
                       STACK_OF(X509) *chain, int build_chain)
{
    ...
    if (chain != NULL && !SSL_CTX_set1_chain(ctx, chain)) {
                             ...
                             unsigned int *cookie_len)
{
    ...
    switch (peer.sa.sa_family) {
        ...
        case AF_INET6:
            ...
            memcpy(buffer + sizeof(peer.s6.sin6_port), &peer.s6.sin6_addr,
...
static int set_cert_cb(SSL *ssl, void *arg)
{
    ...
    for (i = 0; exc != NULL; i++, (exc = exc->prev)) {
        ...
        if (rv & CERT_PKEY_VALID) {
            ...
            /*
             * NB: we wouldn't normally do this as it is not efficient building
             * chains on each connection better to cache the chain in advance.
             */
            if (exc->build_chain) {
...
{
    SSL_EXCERT *exc;
    exc = calloc(1, sizeof(SSL_EXCERT));
    if (exc == NULL)
        return 0;
...
int load_excert(SSL_EXCERT **pexc, BIO *err)
{
    ...
    for (; exc; exc = exc->next) {
        ...
        exc->cert = load_cert(err, exc->certfile, exc->certform, NULL, NULL,
                              "Server Certificate");
        if (!exc->cert)
            return 0;
        if (exc->keyfile) {
...
{
    const uint8_t *rlist;
    static const uint8_t scsv_id[] = { 0, 0, 0xFF };
    ...
    for (i = 0; i < rlistlen; i += num, rlist += num) {
        ...
        else if (memcmp(rlist, scsv_id - num + 3, num) == 0)
            BIO_puts(bio, "SCSV");
        else {
            size_t j;
            BIO_puts(bio, "0x");
