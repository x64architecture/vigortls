...
int cms_main(int argc, char **argv)
{
        ...
        switch (o) {
            ...
            case OPT_KEYOPT:
                ...
                if (key_param == NULL || key_param->idx != keyidx) {
                    ...
                    nparam = malloc(sizeof(cms_key_param));
                    if (!nparam) {
    ...
    if (operation == SMIME_ENCRYPT) {
        ...
        while (*argv) {
            if (!(cert =
                      load_cert(*argv, FORMAT_PEM, NULL, e, "recipient certificate file")))
                goto end;
            sk_X509_push(encerts, cert);
            cert = NULL;
            argv++;
    ...
    if (certfile) {
        if (!(other = load_certs(certfile, FORMAT_PEM, NULL, e, "certificate file"))) {
    ...
    if (operation == SMIME_DATA_CREATE) {
        ...
        if (secret_key) {
            if (!CMS_add0_recipient_key(cms, NID_undef, secret_key, secret_keylen,
                                        secret_keyid, secret_keyidlen, NULL, NULL, NULL))
                goto end;
        ...
        cms = CMS_EncryptedData_encrypt(in, cipher, secret_key, secret_keylen, flags);
        ...
        for (i = 0; i < sk_OPENSSL_STRING_num(sksigners); i++) {
            ...
            signer = load_cert(signerfile, FORMAT_PEM, NULL, e, "signer certificate");
            if (!signer)
                goto end;
            key = load_key(keyfile, keyform, 0, passin, e, "signing key file");
            if (!key)
