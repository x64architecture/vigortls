...
int cms_main(int argc, char **argv)
{
    ...
    if (operation & SMIME_SIGNERS) {
        ...
        if (!recipfile && !keyfile && !secret_key && !pwri_pass && !encerts) {
    ...
    if (operation == SMIME_ENCRYPT) {
        ...
        if (*args && !encerts)
            encerts = sk_X509_new_null();
        while (*args) {
            if (!(cert = load_cert(bio_err, *args, FORMAT_PEM, NULL, e,
                                   "recipient certificate file")))
                goto end;
            sk_X509_push(encerts, cert);
            cert = NULL;
    ...
    if (certfile) {
        if (!(other = load_certs(bio_err, certfile, FORMAT_PEM, NULL, e,
                                 "certificate file"))) {
    ...
    if (recipfile && (operation == SMIME_DECRYPT)) {
        if (!(recip = load_cert(bio_err, recipfile, FORMAT_PEM, NULL, e,
                                "recipient certificate file"))) {
            ERR_print_errors(bio_err);
            goto end;
        }
    ...
    if (operation == SMIME_SIGN_RECEIPT) {
        if (!(signer = load_cert(bio_err, signerfile, FORMAT_PEM, NULL, e,
    ...
    if (operation == SMIME_DATA_CREATE) {
        ...
        if (pwri_pass) {
            pwri_tmp = (uint8_t *)strdup((const char *)pwri_pass);
            if (!pwri_tmp)
                goto end;
        ...
        for (i = 0; i < sk_OPENSSL_STRING_num(sksigners); i++) {
            ...
            signer = load_cert(bio_err, signerfile, FORMAT_PEM, NULL, e,
                               "signer certificate");
            if (!signer)
                goto end;
            key = load_key(bio_err, keyfile, keyform, 0, passin, e,
