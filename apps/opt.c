...
int opt_format(const char *s, unsigned long flags, int *result)
{
    switch (*s) {
        ...
        case 'P':
        case 'p':
            if (s[1] == '\0' || strcasecmp(s, "PEM") == 0) {
                if ((flags & OPT_FMT_PEMDER) == 0)
                    return opt_format_error(s, flags);
                *result = FORMAT_PEM;
            } else if (strcasecmp(s, "PVK") == 0) {
                if ((flags & OPT_FMT_PVK) == 0)
                    return opt_format_error(s, flags);
                *result = FORMAT_PVK;
            } else if (strcasecmp(s, "P12") == 0 || strcasecmp(s, "PKCS12") == 0) {
                if ((flags & OPT_FMT_PKCS12) == 0)
                    return opt_format_error(s, flags);
                *result = FORMAT_PKCS12;
            } else
...
int opt_long(const char *value, long *result)
{
    ...
    *result = strtol(value, &endptr, base);
    if (*endptr) {
        BIO_printf(bio_err, "%s: Bad char %c in number %s\n", prog, *endptr, value);
        return 0;
    }
    return 1;
}
...
int opt_next(void)
{
    ...
    for (o = opts; o->name; ++o) {
        ...
        switch (o->valtype) {
            ...
            case '<':
                /* Input file. */
                if (strcmp(arg, "-") == 0 || access(arg, R_OK) >= 0)
                    break;
                BIO_printf(bio_err, "%s: Cannot open input file %s, %s\n", prog, arg,
                           strerror(errno));
                return -1;
            case '>':
                /* Output file. */
                if (strcmp(arg, "-") == 0 || access(arg, W_OK) >= 0 || errno == ENOENT)
                    break;
                BIO_printf(bio_err, "%s: Cannot open output file %s, %s\n", prog, arg,
                           strerror(errno));
                return -1;
            case 'p':
            case 'n':
                ...
                if (*endptr == '\0') {
                    if (o->valtype == 'p' && val <= 0) {
                        BIO_printf(bio_err, "%s: Non-positive number \"%s\" for -%s\n",
                                   prog, arg, o->name);
                        return -1;
                    }
                    break;
            ...
            case 'f':
            case 'F':
                if (opt_format(arg, o->valtype == 'F' ? OPT_FMT_PEMDER : OPT_FMT_ANY,
                               &dummy))
                    break;
