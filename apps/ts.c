...
static int reply_command(CONF *conf, char *section, char *engine, char *queryfile,
                         char *passin, char *inkey, char *signer, char *chain,
                         const char *policy, char *in, int token_in, char *out,
                         int token_out, int text);
static TS_RESP *read_PKCS7(BIO *in_bio);
static TS_RESP *create_response(CONF *conf, const char *section, char *engine,
                                char *queryfile, char *passin, char *inkey, char *signer,
...
static int verify_command(char *data, char *digest, char *queryfile, char *in, int token_in,
                          char *CApath, char *CAfile, char *untrusted);
static TS_VERIFY_CTX *create_verify_ctx(char *data, char *digest, char *queryfile,
                                        char *CApath, char *CAfile, char *untrusted);
...
static const char *opt_helplist[] = {
    ...
    "          [-text] [-engine id]",
#endif
    "  or", "ts -verify -CApath dir -CAfile file.pem -untrusted file.pem",
    "           [-data file] [-digest hexstring]",
    "           [-queryfile file] -in file [-token_in]", NULL,
};
...
int ts_main(int argc, char **argv)
{
    ...
    switch (mode) {
        ...
        case OPT_REPLY:
            ...
            ret = !reply_command(conf, section, engine, queryfile, password, inkey, signer,
                                 chain, policy, in, token_in, out, token_out, text);
            break;
        case OPT_VERIFY:
            ret = !(((queryfile && !data && !digest) || (!queryfile && data && !digest) ||
...
static CONF *load_config_file(const char *configfile)
{
    ...
    if (configfile &&
        (!(conf = NCONF_new(NULL)) || NCONF_load(conf, configfile, &errorline) <= 0)) {
        if (errorline <= 0)
                         ...
                         int token_out, int text)
{
    ...
    if (in != NULL) {
        ...
        response = create_response(conf, section, engine, queryfile, passin, inkey, signer,
                                   chain, policy);
        if (response)
...
static ASN1_INTEGER *serial_cb(TS_RESP_CTX *ctx, void *data)
{
    ...
    if (!serial) {
        TS_RESP_CTX_set_status_info(ctx, TS_STATUS_REJECTION, "Error during serial number "
                                                              "generation.");
                          ...
                          char *CApath, char *CAfile, char *untrusted)
{
    ...
    if (!(verify_ctx =
              create_verify_ctx(data, digest, queryfile, CApath, CAfile, untrusted)))
        goto end;
