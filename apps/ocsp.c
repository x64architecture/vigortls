...
static int make_ocsp_response(OCSP_RESPONSE **resp, OCSP_REQUEST *req,
                              CA_DB *db, X509 *ca, X509 *rcert, EVP_PKEY *rkey,
                              const EVP_MD *md, STACK_OF(X509) *rother,
...
int ocsp_main(int argc, char **argv)
{
    ...
    if (badarg) {
        ...
        BIO_printf(bio_err, "-sign_other file   additional certificates to "
                            "include in signed request\n");
        BIO_printf(bio_err, "-no_certs          don't include any certificates "
                            "in signed request\n");
        ...
        BIO_printf(bio_err, "-no_intern         don't search certificates "
                            "contained in response for signer\n");
        BIO_printf(bio_err,
                   "-no_signature_verify don't check signature on response\n");
        BIO_printf(bio_err,
                   "-no_cert_verify    don't check signing certificate\n");
        BIO_printf(bio_err, "-no_chain          don't chain verify response\n");
        BIO_printf(bio_err, "-no_cert_checks    don't do additional checks on "
                            "signing certificate\n");
        ...
        BIO_printf(bio_err, "-resp_key_id       identify response by signing "
                            "certificate key ID\n");
...
static int make_ocsp_response(OCSP_RESPONSE **resp, OCSP_REQUEST *req,
                              CA_DB *db, X509 *ca, X509 *rcert, EVP_PKEY *rkey,
                              const EVP_MD *rmd, STACK_OF(X509) *rother,
                              unsigned long flags, int nmin, int ndays,
                              int badsig)
{
...
static char **lookup_serial(CA_DB *db, ASN1_INTEGER *ser)
{
    ...
    OPENSSL_assert(
        bn); /* FIXME: should report an error at this point and abort */
    if (BN_is_zero(bn))
        itmp = strdup("00");
    else
        itmp = BN_bn2hex(bn);
