    ...
    if (pkey_op == EVP_PKEY_OP_VERIFY) {
        ...
        if (rv > 0 && buf_outlen != 0) {
            ...
            if (!buf_out)
                ...
                rv = do_keyop(ctx, pkey_op, buf_out, (size_t *)&buf_outlen,
...
#ifndef OPENSSL_NO_ENGINE
    BIO_printf(bio_err, "-engine e       use engine e, maybe a hardware device, for loading keys.\n");
                              ...
                              ENGINE *e, int engine_impl)
{
    ...
    switch (key_type) {
        ...
        case KEY_PUBKEY:
            pkey = load_pubkey(bio_err, keyfile, keyform, 0, NULL, e,
                               "Public Key");
            break;
