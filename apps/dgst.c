...
#define PROG_NAME_SIZE 39
    ...
    if (keyfile) {
        if (want_pub)
            sigkey =
                load_pubkey(bio_err, keyfile, keyform, 0, NULL, e, "key file");
        else
            sigkey =
                load_key(bio_err, keyfile, keyform, 0, passin, e, "key file");
        if (!sigkey) {
    ...
    if (hmac_key) {
        sigkey =
            EVP_PKEY_new_mac_key(EVP_PKEY_HMAC, e, (uint8_t *)hmac_key, -1);
        if (!sigkey)
            goto end;
    }
    ...
    if (argc == 0) {
        ...
        err = do_fp(out, buf, inp, separator, out_bin, sigkey, sigbuf, siglen,
                    NULL, NULL, "stdin", bmd);
    } else {
