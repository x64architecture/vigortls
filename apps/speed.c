...
static const char *names[ALGOR_NUM] = {
    "md5", "hmac(md5)", "sha1", "rmd160", "rc4", "des cbc", "des ede3",
    "idea cbc", "rc2 cbc", "rc5-32/12 cbc", "blowfish cbc", "cast cbc",
    "aes-128 cbc", "aes-192 cbc", "aes-256 cbc", "camellia-128 cbc",
    "camellia-192 cbc", "camellia-256 cbc", "evp", "sha256", "sha512",
    "whirlpool", "aes-128 ige", "aes-192 ige", "aes-256 ige", "ghash",
    "chacha20 poly1305",
};
...
int speed_main(int argc, char **argv)
{
    ...
    CAST_KEY cast_ks;
#endif
    static const uint8_t key16[16] = { 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc,
                                       0xde, 0xf0, 0x34, 0x56, 0x78, 0x9a,
                                       0xbc, 0xde, 0xf0, 0x12 };
    static const uint8_t key24[24] = { 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc,
                                       0xde, 0xf0, 0x34, 0x56, 0x78, 0x9a,
                                       0xbc, 0xde, 0xf0, 0x12, 0x56, 0x78,
                                       0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34 };
    static const uint8_t key32[32] = { 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde,
    ...
    static const uint8_t ckey24[24] = { 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc,
                                        ...
                                        0xbc, 0xde, 0xf0, 0x12, 0x56, 0x78,
                                        0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34 };
    static const uint8_t ckey32[32] = {
...
#define D_CBC_RC5      9
#define D_CBC_BF       10
#define D_CBC_CAST     11
#define D_CBC_128_AES  12
#define D_CBC_192_AES  13
#define D_CBC_256_AES  14
#define D_CBC_128_CML  15
#define D_CBC_192_CML  16
#define D_CBC_256_CML  17
#define D_EVP          18
#define D_SHA256       19
#define D_SHA512       20
...
#define R_EC_B571 15
    ...
    static uint8_t *rsa_data[RSA_NUM] = { test512, test1024, test2048,
                                          test4096 };
    ...
    static int test_curves_bits[EC_NUM] = { 160, 192, 224, 256, 384, 521,
                                            163, 233, 283, 409, 571, 163,
                                            233, 283, 409, 571 };
    ...
    while (argc) {
        if ((argc > 0) && (strcmp(*argv, "-elapsed") == 0)) {
            ...
            doit[D_SHA512] = 1;
        else if (strcmp(*argv, "whirlpool") == 0)
            doit[D_WHIRLPOOL] = 1;
        else
#ifndef OPENSSL_NO_RIPEMD
        ...
        else if (strcmp(*argv, "camellia-128-cbc") == 0)
            ...
            doit[D_GHASH] = 1;
        } else if (strcmp(*argv, "camellia") == 0) {
            doit[D_CBC_128_CML] = 1;
            doit[D_CBC_192_CML] = 1;
            doit[D_CBC_256_CML] = 1;
        } else
        ...
        if (strcmp(*argv, "rsa") == 0) {
            ...
            BIO_printf(bio_err, "-elapsed        measure time in real time "
    ...
    if (usertime == 0 && !mr)
        BIO_printf(bio_err, "You have chosen to measure elapsed time instead "
                            "of user CPU time.\n");
    ...
    do {
        ...
        Time_F(START);
        for (it = count; it; it--)
            DES_ecb_encrypt((DES_cblock *)buf, (DES_cblock *)buf, &sch,
                            DES_ENCRYPT);
        d = Time_F(STOP);
    } while (d < 3);
    save_count = count;
    ...
    if (doit[D_CBC_DES]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_DES][j]); count++)
                DES_ncbc_encrypt(buf, buf, lengths[j], &sch, &DES_iv,
                                 DES_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_EDE3_DES]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_EDE3_DES][j]); count++)
                DES_ede3_cbc_encrypt(buf, buf, lengths[j], &sch, &sch2, &sch3,
                                     &DES_iv, DES_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_CBC_128_AES]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_128_AES][j]); count++)
                AES_cbc_encrypt(buf, buf, (unsigned long)lengths[j], &aes_ks1,
                                iv, AES_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_CBC_192_AES]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_192_AES][j]); count++)
                AES_cbc_encrypt(buf, buf, (unsigned long)lengths[j], &aes_ks2,
                                iv, AES_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_CBC_256_AES]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_256_AES][j]); count++)
                AES_cbc_encrypt(buf, buf, (unsigned long)lengths[j], &aes_ks3,
                                iv, AES_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_IGE_128_AES]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_IGE_128_AES][j]); count++)
                AES_ige_encrypt(buf, buf2, (unsigned long)lengths[j], &aes_ks1,
                                iv, AES_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_IGE_192_AES]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_IGE_192_AES][j]); count++)
                AES_ige_encrypt(buf, buf2, (unsigned long)lengths[j], &aes_ks2,
                                iv, AES_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_IGE_256_AES]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_IGE_256_AES][j]); count++)
                AES_ige_encrypt(buf, buf2, (unsigned long)lengths[j], &aes_ks3,
                                iv, AES_ENCRYPT);
            d = Time_F(STOP);
      ...
      if (doit[D_CHACHA20_POLY1305]) {
          ...
          for (j = 0; j < SIZE_NUM; j++) {
               ...
               for (count = 0, run = 1; COND(c[D_CHACHA20_POLY1305][j]); count++)
                    EVP_AEAD_CTX_seal(&ctx, buf, &out_len, BUFSIZE, nonce,
    ...
    if (doit[D_CBC_128_CML]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_128_CML][j]); count++)
                Camellia_cbc_encrypt(buf, buf, (unsigned long)lengths[j],
                                     &camellia_ks1, iv, CAMELLIA_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_CBC_192_CML]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_192_CML][j]); count++)
                Camellia_cbc_encrypt(buf, buf, (unsigned long)lengths[j],
                                     &camellia_ks2, iv, CAMELLIA_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_CBC_256_CML]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_256_CML][j]); count++)
                Camellia_cbc_encrypt(buf, buf, (unsigned long)lengths[j],
                                     &camellia_ks3, iv, CAMELLIA_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_CBC_IDEA]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_IDEA][j]); count++)
                idea_cbc_encrypt(buf, buf, (unsigned long)lengths[j], &idea_ks,
                                 iv, IDEA_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_CBC_RC2]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_RC2][j]); count++)
                RC2_cbc_encrypt(buf, buf, (unsigned long)lengths[j], &rc2_ks,
                                iv, RC2_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_CBC_RC5]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_RC5][j]); count++)
                RC5_32_cbc_encrypt(buf, buf, (unsigned long)lengths[j], &rc5_ks,
                                   iv, RC5_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_CBC_BF]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_BF][j]); count++)
                BF_cbc_encrypt(buf, buf, (unsigned long)lengths[j], &bf_ks, iv,
                               BF_ENCRYPT);
            d = Time_F(STOP);
    ...
    if (doit[D_CBC_CAST]) {
        for (j = 0; j < SIZE_NUM; j++) {
            ...
            for (count = 0, run = 1; COND(c[D_CBC_CAST][j]); count++)
                CAST_cbc_encrypt(buf, buf, (unsigned long)lengths[j], &cast_ks,
                                 iv, CAST_ENCRYPT);
            d = Time_F(STOP);
    ...
    for (j = 0; j < RSA_NUM; j++) {
        ...
        if (ret == 0) {
            ...
            for (count = 0, run = 1; COND(rsa_c[j][0]); count++) {
                ret =
                    RSA_sign(NID_md5_sha1, buf, 36, buf2, &rsa_num, rsa_key[j]);
                if (ret == 0) {
    ...
    for (j = 0; j < EC_NUM; j++) {
        ...
        if (ecdsa[j] == NULL) {
            ...
            if (ret == 0) {
                ...
                pkey_print_message("sign", "ecdsa", ecdsa_c[j][0],
                                   test_curves_bits[j], ECDSA_SECONDS);

                Time_F(START);
                for (count = 0, run = 1; COND(ecdsa_c[j][0]); count++) {
                    ret = ECDSA_sign(0, buf, 20, ecdsasig, &ecdsasiglen,
                                     ecdsa[j]);
                    if (ret == 0) {
            ...
            if (ret != 1) {
                ...
                pkey_print_message("verify", "ecdsa", ecdsa_c[j][1],
                                   test_curves_bits[j], ECDSA_SECONDS);
                Time_F(START);
    ...
    for (j = 0; j < EC_NUM; j++) {
        ...
        if ((ecdh_a[j] == NULL) || (ecdh_b[j] == NULL)) {
            ...
            if (!EC_KEY_generate_key(ecdh_a[j]) ||
                ...
                /* If field size is not more than 24 octets, then use SHA-1 hash
                 * of result;
                 * otherwise, use result (see section 4.8 of
                ...
                secret_size_a = ECDH_compute_key(
                    secret_a, outlen, EC_KEY_get0_public_key(ecdh_b[j]),
                    ecdh_a[j], kdf);
                secret_size_b = ECDH_compute_key(
                    secret_b, outlen, EC_KEY_get0_public_key(ecdh_a[j]),
                    ecdh_b[j], kdf);
                ...
                for (secret_idx = 0;
                     (secret_idx < secret_size_a) && (ecdh_checks == 1);
                     secret_idx++) {
    ...
    for (k = 0; k < EC_NUM; k++) {
        ...
        if (mr)
            ...
            fprintf(stdout, "%4u bit ecdsa (%s) %8.4fs %8.4fs %8.1f %8.1f\n",
                    test_curves_bits[k], test_curves_names[k],
                    ecdsa_results[k][0], ecdsa_results[k][1],
