                                         ...
                                         int n4x) /* n4x is 1 or 2 */
{
    ...
    CIPH_DESC ciph_d[8];
    uint8_t storage[sizeof(SHA1_MB_CTX) + 32];
    union {
        uint64_t q[16];
        uint32_t d[32];
        uint8_t c[128];
    ...
    hash_d[0].ptr = inp;
    for (i = 1; i < x4; i++)
        hash_d[i].ptr = hash_d[i - 1].ptr + frag;
...
#else
        blocks[i].c[7] += ((uint8_t *)key->md.data)[7] + i;
        if (blocks[i].c[7] < i) {
            int j;
            
            for (j = 6; j >= 0; j--) {
                if (blocks[i].c[j] = ((uint8_t *)key->md.data)[j] + 1)
                    break;
            }
        }
#endif
    ...
    for (i = x4 - 1;; i--) {
        ...
        /* write MAC */
        ((uint32_t *)out)[0] = BSWAP4(ctx->A[i]);
        ((uint32_t *)out)[1] = BSWAP4(ctx->B[i]);
        ((uint32_t *)out)[2] = BSWAP4(ctx->C[i]);
        ((uint32_t *)out)[3] = BSWAP4(ctx->D[i]);
        ((uint32_t *)out)[4] = BSWAP4(ctx->E[i]);
        out += 20;
        len += 20;
                                      ...
                                      const uint8_t *in, size_t len)
{
    ...
    if (ctx->encrypt) {
        ...
        if (plen != NO_PAYLOAD_LENGTH) { /* "TLS" mode of operation */
            ...
            uint8_t tail_iv[AES_BLOCK_SIZE], stitch = 0;
#endif
                ...
                >= TLS1_1_VERSION)
            {
                ...
                out += AES_BLOCK_SIZE;
                len -= AES_BLOCK_SIZE;
            } else if (iv < (SHA_DIGEST_LENGTH + 1))
                return 0;
