...
#define TS_FAILURE_INFO_SIZE (sizeof(TS_failure_info) / sizeof(*TS_failure_info))
...
static int TS_check_status_info(TS_RESP *response)
{
    ...
    ERR_asprintf_error_data("status code: %s, status text: %s, failure codes: %s",
                            status_text, embedded_status_text ? embedded_status_text : "unspecified",
                            failure_text);
                             ...
                             TS_TST_INFO *tst_info)
{
    ...
    ret = len_a == (unsigned)ASN1_STRING_length(b->hashed_msg) && memcmp(imprint_a, ASN1_STRING_data(b->hashed_msg), len_a) == 0;
err:
    if (!ret)
