...
char *i2s_ASN1_ENUMERATED(X509V3_EXT_METHOD *method, ASN1_ENUMERATED *a)
{
    ...
    if (!a)
        return NULL;
    if (!(bntmp = ASN1_ENUMERATED_to_BN(a, NULL)) || !(strtmp = BN_bn2dec(bntmp)))
...
char *i2s_ASN1_INTEGER(X509V3_EXT_METHOD *method, ASN1_INTEGER *a)
{
    ...
    if (!a)
        return NULL;
    if (!(bntmp = ASN1_INTEGER_to_BN(a, NULL)) || !(strtmp = BN_bn2dec(bntmp)))
...
int X509V3_get_value_bool(CONF_VALUE *value, int *asn1_bool)
{
    ...
    if (strcmp(btmp, "TRUE") == 0 || strcmp(btmp, "true") == 0 ||
        ...
        *asn1_bool = 0xff;
        return 1;
    } else if (strcmp(btmp, "FALSE") == 0 || strcmp(btmp, "false") == 0 ||
...
typedef int (*equal_fn)(const uint8_t *pattern, size_t pattern_len,
                        const uint8_t *subject, size_t subject_len,
                        unsigned int flags);

/* Skip pattern prefix to match "wildcard" subject */
static void skip_prefix(const uint8_t **p, size_t *plen, const uint8_t *subject,
                        size_t subject_len, unsigned int flags)
{
    const uint8_t *pattern = *p;
     ...
     * set by the user).
     */
    if ((flags & _X509_CHECK_FLAG_DOT_SUBDOMAINS) == 0 || subject_len <= 1 ||
        subject[0] != '.')
        return;
...
static int equal_nocase(const uint8_t *pattern, size_t pattern_len,
                        const uint8_t *subject, size_t subject_len,
                        unsigned int flags)
{
...
static int equal_case(const uint8_t *pattern, size_t pattern_len,
                      const uint8_t *subject, size_t subject_len,
                      unsigned int flags)
{
 ...
 */
static int equal_email(const uint8_t *a, size_t a_len, const uint8_t *b,
...
static int wildcard_match(const uint8_t *prefix, size_t prefix_len,
                          const uint8_t *suffix, size_t suffix_len,
                          const uint8_t *subject, size_t subject_len,
                          unsigned int flags)
{
    const uint8_t *wildcard_start;
    const uint8_t *wildcard_end;
    const uint8_t *p;
    int allow_multi = 0;
    int allow_idna = 0;
...
static const uint8_t *valid_star(const uint8_t *p, size_t len,
                                 unsigned int flags)
{
...
static int equal_wildcard(const uint8_t *pattern, size_t pattern_len,
                          const uint8_t *subject, size_t subject_len,
                          unsigned int flags)
{
...
static int do_check_string(ASN1_STRING *a, int cmp_type, equal_fn equal,
                           unsigned int flags, const uint8_t *b, size_t blen)
{
    if (!a->data || !a->length)
        return 0;
    if (cmp_type > 0) {
        ...
        if (a->length == (int)blen && memcmp(a->data, b, blen) == 0)
            return 1;
        else
            return 0;
    } else {
...
static int do_x509_check(X509 *x, const uint8_t *chk, size_t chklen,
                         unsigned int flags, int check_type)
{
    ...
    if (gens) {
        ...
        for (i = 0; i < sk_GENERAL_NAME_num(gens); i++) {
            ...
            if (do_check_string(cstr, alt_type, equal, flags, chk, chklen)) {
                rv = 1;
                break;
            }
        }
...
int X509_check_host(X509 *x, const uint8_t *chk, size_t chklen,
                    unsigned int flags)
{
    return do_x509_check(x, chk, chklen, flags, GEN_DNS);
}

int X509_check_email(X509 *x, const uint8_t *chk, size_t chklen,
                     unsigned int flags)
{
                             ...
                             unsigned long chtype)
{
    ...
    for (i = 0; i < sk_CONF_VALUE_num(dn_sk); i++) {
        ...
        if (*type == '+') {
            ...
            mval = 0;
        if (!X509_NAME_add_entry_by_txt(nm, type, chtype, (uint8_t *)v->value,
                                        -1, -1, mval))
            return 0;
    }
    return 1;
