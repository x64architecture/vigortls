...
err:
    ...
    if (buf_in != NULL) {
        vigortls_zeroize((char *)buf_in, (unsigned int)inl);
        free(buf_in);
    }
    if (buf_out != NULL) {
        vigortls_zeroize((char *)buf_out, outll);
        free(buf_out);
    }
    return (outl);
}
                       ...
                       ASN1_BIT_STRING *signature, void *asn, EVP_MD_CTX *ctx)
{
    ...
    if (rv == 2) {
        if (type->flags & EVP_MD_FLAG_PKEY_METHOD_SIGNATURE) {
            if (!pkey->ameth || !OBJ_find_sigid_by_algs(&signid, EVP_MD_nid(type), pkey->ameth->pkey_id)) {
