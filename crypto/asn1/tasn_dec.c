...
static int asn1_check_eoc(const uint8_t **in, long len);
static int asn1_find_end(const uint8_t **in, long len, char inf);

static int asn1_collect(BUF_MEM *buf, const uint8_t **in, long len,
                        char inf, int tag, int aclass, int depth);

static int collect_data(BUF_MEM *buf, const uint8_t **p, long plen);

static int asn1_check_tlen(long *olen, int *otag, uint8_t *oclass,
                           char *inf, char *cst,
...
static int asn1_ex_c2i(ASN1_VALUE **pval, const uint8_t *cont, int len, int utype,
...
int ASN1_item_ex_d2i(ASN1_VALUE **pval, const uint8_t **in, long len,
                     const ASN1_ITEM *it,
                     int tag, int aclass, char opt, ASN1_TLC *ctx)
{
    ...
    switch (utype) {
        ...
        default:
            ...
            if (*free_cont) {
                if (stmp->data)
                    free(stmp->data);
                stmp->data = (uint8_t *)cont; /* UGLY CAST! RL */
                stmp->length = len;
                *free_cont = 0;
            } else {
