...
int dsa_builtin_paramgen2(DSA *ret, size_t L, size_t N, const EVP_MD *evpmd,
                          const uint8_t *seed_in, size_t seed_len, int idx,
                          uint8_t *seed_out, int *counter_ret,
                          unsigned long *h_ret, BN_GENCB *cb)
{
    int ok = -1;
    uint8_t *seed = NULL, *seed_tmp = NULL;
...
g_only:
    ...
    for (;;) {
        static const uint8_t ggen[4] = { 0x67, 0x67, 0x65, 0x6e };
        if (idx >= 0) {
            md[0] = idx & 0xff;
            md[1] = (h >> 8) & 0xff;
            md[2] = h & 0xff;
