...
{
    SHA512_CTX ctx;
    static uint8_t digest[SHA384_DIGEST_LENGTH];
...
int SHA512_Final(uint8_t *md, SHA512_CTX *ctx)
{
    ...
    n++;
    if (n > (sizeof(ctx->u) - 16)) {
        memset(p + n, 0, sizeof(ctx->u) - n);
        n = 0;
        sha512_block_data_order(ctx, p, 1);
    }

    memset(p + n, 0, sizeof(ctx->u) - 16 - n);
    p[sizeof(ctx->u) - 1] = (uint8_t)(ctx->Nl);
    p[sizeof(ctx->u) - 2] = (uint8_t)(ctx->Nl >> 8);
    p[sizeof(ctx->u) - 3] = (uint8_t)(ctx->Nl >> 16);
    p[sizeof(ctx->u) - 4] = (uint8_t)(ctx->Nl >> 24);
    p[sizeof(ctx->u) - 5] = (uint8_t)(ctx->Nl >> 32);
    p[sizeof(ctx->u) - 6] = (uint8_t)(ctx->Nl >> 40);
    p[sizeof(ctx->u) - 7] = (uint8_t)(ctx->Nl >> 48);
    p[sizeof(ctx->u) - 8] = (uint8_t)(ctx->Nl >> 56);
    p[sizeof(ctx->u) - 9] = (uint8_t)(ctx->Nh);
    p[sizeof(ctx->u) - 10] = (uint8_t)(ctx->Nh >> 8);
    p[sizeof(ctx->u) - 11] = (uint8_t)(ctx->Nh >> 16);
    p[sizeof(ctx->u) - 12] = (uint8_t)(ctx->Nh >> 24);
    p[sizeof(ctx->u) - 13] = (uint8_t)(ctx->Nh >> 32);
    p[sizeof(ctx->u) - 14] = (uint8_t)(ctx->Nh >> 40);
    p[sizeof(ctx->u) - 15] = (uint8_t)(ctx->Nh >> 48);
    p[sizeof(ctx->u) - 16] = (uint8_t)(ctx->Nh >> 56);
