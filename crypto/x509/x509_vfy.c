...
static int check_id(X509_STORE_CTX *ctx)
{
    ...
    if (id->host && !X509_check_host(x, id->host, id->hostlen, id->hostflags)) {
...
static int check_trust(X509_STORE_CTX *ctx)
{
    ...
    /*
     * If no trusted certs in chain at all return untrusted and allow standard
     * (no issuer cert) etc errors to be indicated.
     */
