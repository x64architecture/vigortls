                          ...
                          const uint8_t *ukm, uint8_t *key)
{
    ...
    if (VKO_compute_key(X, Y, pub_key->pkey.gost, priv_key->pkey.gost, UKM) == 0)
        goto err;
    switch (digest_nid) {
                        ...
                        const uint8_t *in, size_t in_len)
{
    ...
    if (eph_key) {
        if (EVP_PKEY_derive_set_peer(pctx, eph_key) <= 0) {
            GOSTerr(GOST_F_PKEY_GOST01_DECRYPT, GOST_R_INCOMPATIBLE_PEER_KEY);
            goto err;
        }
    } else {
    ...
    if (gost_key_unwrap_crypto_pro(nid, sharedKey, wrappedKey, key) == 0) {
        GOSTerr(GOST_F_PKEY_GOST01_DECRYPT, GOST_R_ERROR_COMPUTING_SHARED_KEY);
        goto err;
    }
...
int pkey_gost01_derive(EVP_PKEY_CTX *ctx, uint8_t *key, size_t *keylen)
{
     ...
     * Our private key is in the ctx pkey
                        ...
                        const uint8_t *key, size_t key_len)
{
    ...
    if (data->shared_ukm != NULL) {
        ...
        if (RAND_bytes(ukm, 8) <= 0) {
            GOSTerr(GOST_F_PKEY_GOST01_ENCRYPT, GOST_R_RANDOM_GENERATOR_FAILURE);
            return 0;
        }
    }
    ...
    if (ASN1_OCTET_STRING_set(gkt->key_info->imit, crypted_key + 40, 4) == 0) {
        goto err;
    }
    if (ASN1_OCTET_STRING_set(gkt->key_info->encrypted_key, crypted_key + 8, 32) == 0) {
        goto err;
    }
    if (key_is_ephemeral) {
        if (X509_PUBKEY_set(&gkt->key_agreement_info->ephem_key,
                            out ? sec_key : pubk) == 0) {
            GOSTerr(GOST_F_PKEY_GOST01_ENCRYPT, GOST_R_CANNOT_PACK_EPHEMERAL_KEY);
            goto err;
        }
    }
    ...
    if (key_is_ephemeral)
        ...
        /* Set control "public key from client certificate used" */
        if (EVP_PKEY_CTX_ctrl(pctx, -1, -1, EVP_PKEY_CTRL_PEER_KEY, 3, NULL) <= 0) {
            GOSTerr(GOST_F_PKEY_GOST01_ENCRYPT, GOST_R_CTRL_CALL_FAILED);
            goto err;
        }
    }
...
static int pkey_gost01_ctrl_str(EVP_PKEY_CTX *ctx, const char *type,
                                const char *value)
{
    ...
    if (strcmp(type, "paramset") == 0) {
        ...
        return pkey_gost01_ctrl(ctx, EVP_PKEY_CTRL_GOST_PARAMSET, param_nid, NULL);
    }
    if (strcmp(type, "dgst") == 0) {
        if (value == NULL)
            return 0;
        else if (strcmp(value, "gost94") == 0 || strcmp(value, "md_gost94") == 0)
        ...
        return pkey_gost01_ctrl(ctx, EVP_PKEY_CTRL_GOST_SET_DIGEST, digest_nid, NULL);
    }
    return -2;
}
