...
void ecp_nistz256_select_w7(P256_POINT_AFFINE *val, const P256_POINT_AFFINE *in_t,
                            int index);
                                   ...
                                   const P256_POINT *b)
{
    ...
    in1infty = in1_x[0] | in1_x[1] | in1_x[2] | in1_x[3] | in1_y[0] | in1_y[1]
               | in1_y[2] | in1_y[3];
    if (P256_LIMBS == 8)
        in1infty |= in1_x[4] | in1_x[5] | in1_x[6] | in1_x[7] | in1_y[4] | in1_y[5]
                    | in1_y[6] | in1_y[7];

    in2infty = in2_x[0] | in2_x[1] | in2_x[2] | in2_x[3] | in2_y[0] | in2_y[1]
               | in2_y[2] | in2_y[3];
    if (P256_LIMBS == 8)
        in2infty |= in2_x[4] | in2_x[5] | in2_x[6] | in2_x[7] | in2_y[4] | in2_y[5]
                    | in2_y[6] | in2_y[7];
                                     ...
                                     int num, BN_CTX *ctx)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        /* This is an unusual input, we *do not* guarantee constant-timeness */
...
static int ecp_nistz256_mult_precompute(EC_GROUP *group, BN_CTX *ctx)
{
    ...
    for (k = 0; k < 64; k++) {
        ...
        for (j = 0; j < 37; j++) {
            /* 
             * It would be faster to use EC_POINTs_make_affine and make multiple
             * points affine at the same time.
             */
            if (!EC_POINT_make_affine(group, P, ctx))
                goto err;
            if (!ecp_nistz256_bignum_to_field_elem(preComputedTable[j][k].X, &P->X) ||
                !ecp_nistz256_bignum_to_field_elem(preComputedTable[j][k].Y, &P->Y)) {
                                    ...
                                    const P256_POINT_AFFINE (*preComputedTable)[64])
{
    ...
    ecp_nistz256_avx2_multi_select_w7(point_arr, preComputedTable[0], digit0,
    ...
    ecp_nistz256_avx2_multi_select_w7(point_arr, preComputedTable[4 * 1], digit0,
                                      digit1, digit2, digit3);
    ...
    for (i = 2; i < 9; i++) {
        ...
        ecp_nistz256_avx2_multi_select_w7(point_arr, preComputedTable[4 * i],
    ...
    wvalue = (wvalue >> ((index - 1) % 8)) & mask;
    booth_recode_w7(&sign0, &digit0, wvalue);
    ecp_nistz256_avx2_select_w7((P256_POINT_AFFINE *)r, preComputedTable[36],
                                digit0);
                                   ...
                                   const BIGNUM *scalars[], BN_CTX *ctx)
{
    ...
    if (scalar) {
        ...
        if (pre_comp) {
            ...
            if (!ecp_nistz256_set_from_affine(pre_comp_generator, group,
                                              pre_comp->precomp[0], ctx)) {
...
const EC_METHOD *EC_GFp_nistz256_method(void)
{
    static const EC_METHOD ret = { 
            EC_FLAGS_DEFAULT_OCT, NID_X9_62_prime_field, ec_GFp_mont_group_init,
            ec_GFp_mont_group_finish, ec_GFp_mont_group_clear_finish,
            ec_GFp_mont_group_copy, ec_GFp_mont_group_set_curve,
            ...
            ec_GFp_simple_point_set_affine_coordinates, ecp_nistz256_get_affine, 0,
            0, 0, ec_GFp_simple_add, ec_GFp_simple_dbl, ec_GFp_simple_invert,
            ec_GFp_simple_is_at_infinity, ec_GFp_simple_is_on_curve,
            ec_GFp_simple_cmp, ec_GFp_simple_make_affine,
            ec_GFp_simple_points_make_affine, ecp_nistz256_points_mul, /* mul */
            ecp_nistz256_mult_precompute,             /* precompute_mult */
            ecp_nistz256_window_have_precompute_mult, /* have_precompute_mult */
            ec_GFp_mont_field_mul, ec_GFp_mont_field_sqr, 0, /* field_div */
            ec_GFp_mont_field_encode, ec_GFp_mont_field_decode,
