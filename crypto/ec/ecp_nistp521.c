...
/*
 * felem_diff_128 subtracts |in| from |out|
 * On entry:
 *   in[i] < 2^126
 * On exit:
...
{
    /* In order to prevent underflow, we add 0 mod p before subtracting. */
    static const uint128_t two127m70 = (((uint128_t)1) << 127) -
        (((uint128_t)1) << 70);
    static const uint128_t two127m69 = (((uint128_t)1) << 127) -
        (((uint128_t)1) << 69);
...
static NISTP521_PRE_COMP *nistp521_pre_comp_new(void)
{
    ...
    ret = calloc(1, sizeof(NISTP521_PRE_COMP));
    if (ret == NULL) {
                               ...
                               const BIGNUM *scalars[], BN_CTX *ctx)
{
    ...
    if (scalar != NULL) {
        ...
        if (!EC_POINT_set_Jprojective_coordinates_GFp(group, generator,
                                                      x, y, z, ctx))
            goto err;
    ...
    if (num_points > 0) {
        ...
        if (mixed)
            tmp_felems = reallocarray(NULL, (num_points * 17 + 1), sizeof(felem));
...
int ec_GFp_nistp521_have_precompute_mult(const EC_GROUP *group)
{
    if (EC_EX_DATA_get_data(group->extra_data, nistp521_pre_comp_dup,
                            nistp521_pre_comp_free,
                            nistp521_pre_comp_clear_free) != NULL)
        return 1;
    else
        return 0;
}
