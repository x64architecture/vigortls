...
static int test(void)
{
    uint8_t cbc_in[40], cbc_out[40], iv[8];
    int i, n, err = 0;
    BF_KEY key;
    BF_LONG data[2];
    uint8_t out[8];
    ...
    for (n = 0; n < 2; n++) {
        BF_set_key(&key, strlen(bf_key[n]), (uint8_t *)bf_key[n]);
    ...
    BF_cbc_encrypt((uint8_t *)cbc_data, cbc_out, len,
                   &key, iv, BF_ENCRYPT);
