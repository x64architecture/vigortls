...
#define CHAR2_CURVE_TEST_INTERNAL(_name, _p, _a, _b, _x, _y, _y_bit, _order, _cof, _degree, _variable) \
    if (!BN_hex2bn(&x, _x))                                                                            \
        ABORT;                                                                                         \
    if (!EC_POINT_set_compressed_coordinates_GF2m(group, P, x, _y_bit, ctx))                           \
        ABORT;                                                                                         \
    if (EC_POINT_is_on_curve(group, P, ctx) <= 0)                                                      \
        ABORT;                                                                                         \
    if (!BN_hex2bn(&z, _order))                                                                        \
        ABORT;                                                                                         \
    if (!BN_hex2bn(&cof, _cof))                                                                        \
        ABORT;                                                                                         \
    if (!EC_GROUP_set_generator(group, P, z, cof))                                                     \
        ABORT;                                                                                         \
...
#define CHAR2_CURVE_TEST(_name, _p, _a, _b, _x, _y, _y_bit, _order, _cof, _degree, _variable)      \
    ...
    fprintf(stdout, "verify degree ...");                                                          \
    if (EC_GROUP_get_degree(group) != _degree)                                                     \
        ABORT;                                                                                     \
    fprintf(stdout, " ok\n");                                                                      \
    group_order_tests(group);                                                                      \
    if (!(_variable = EC_GROUP_new(EC_GROUP_method_of(group))))                                    \
        ABORT;                                                                                     \
    if (!EC_GROUP_copy(_variable, group))                                                          \
        ABORT;
