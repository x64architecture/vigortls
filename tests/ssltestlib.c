...
static BIO_METHOD method_tls_dump = {
    .type = BIO_TYPE_TLS_DUMP_FILTER,
    .name = "TLS dump filter",
    .bwrite = tls_dump_write,
    .bread = tls_dump_read,
    .bputs = tls_dump_puts,
    .bgets = tls_dump_gets,
    .ctrl = tls_dump_ctrl,
    .create = tls_dump_new,
    .destroy = tls_dump_free
};
...
static BIO_METHOD method_mempacket_test = {
    .type = BIO_TYPE_MEMPACKET_TEST,
    .name = "Mem Packet Test",
    .bwrite = mempacket_test_write,
    .bread = mempacket_test_read,
    .bputs = mempacket_test_puts,
    .bgets = mempacket_test_gets,
    .ctrl = mempacket_test_ctrl,
    .create = mempacket_test_new,
    .destroy = mempacket_test_free,
};
...
{
    MEMPACKET_TEST_CTX *ctx = calloc(1, sizeof(*ctx));
    if (ctx == NULL)
        return 0;
                          ...
                          int type)
{
    ...
    thispkt = malloc(sizeof(MEMPACKET));
    if (thispkt == NULL)
        return -1;

    thispkt->data = malloc(inl);
